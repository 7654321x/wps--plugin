...
interface ApplicationLike { ActiveDocument?: { FullName?: string; Saved?: boolean; Save?: () => void; Paragraphs?: { Count?: number; Item?: (index: number) => { Range?: { Text?: string } } } }; PluginStorage: StorageLike; CreateTaskPane(url: string, title?: string): TaskPaneLike; GetTaskPane(id: number | string): TaskPaneLike; }
...
export class HostResultStore {
  ...
  constructor(private readonly storage: StorageLike, private readonly build: BuildInfo, readonly hostContextId = crypto.randomUUID()) {
    ...
    this.state = stored?.build_id === build.build_id ? { ...stored, host_context_id: hostContextId } : { schema_version: 1, build_id: build.build_id, asset_hash: build.asset_hash, host_context_id: hostContextId, document_identity_hash: "", active_command: null, command_status: "IDLE", active_view: "execution", recognition_summary: "", paragraph_recognition_models: [], formatting_preview_models: [], preview_comment_status: "", formatting_progress: "就绪", formatting_result: "", latest_error: stored ? "ADDIN_CONTEXT_STALE" : "", callback_log: [], updated_at: now() };
    this.save();
  }
...
export class HostCommandRouter {
  ...
  constructor(private readonly app: ApplicationLike, private readonly panes: TaskPaneManager, private readonly store: HostResultStore, private readonly config: ClassifiedRuntimeConfig) {
    this.registry = { recognize_document: (result) => this.recognize(result), preview_document: (result) => this.preview(result), clear_preview: () => this.clearPreview(), format_document: (result) => this.format(result), open_taskpane: async () => { this.panes.show(); return "任务窗格已打开"; }, close_taskpane: async () => { this.panes.hide(); return "任务窗格已关闭"; }, toggle_taskpane: async () => { const pane = this.panes.toggle(); return pane?.Visible ? "任务窗格已打开" : "任务窗格已关闭"; }, show_about: async () => { this.panes.show(); this.store.update({ active_view: "issues", latest_error: "Docxtool 涉密版，仅连接本机服务。" }); return "关于信息已显示"; } };
  }
  async dispatch(name: HostCommandName, source: "ribbon" | "taskpane" | "e2e" = "ribbon", requestId = id("host"), taskpaneBuildId = this.store.read().build_id): Promise<CommandResult> {
    ...
    const started = now(); const view = name === "recognize_document" ? "recognition" : name === "preview_document" ? "preview" : name === "format_document" ? "execution" : this.store.read().active_view; const result = this.store.begin(name, requestId, view);
    try {
      if (taskpaneBuildId !== this.store.read().build_id) throw new Error("ADDIN_CONTEXT_STALE");
      if (["recognize_document", "preview_document", "format_document"].includes(name)) this.panes.show();
  ...
  async reconcileActiveDocument(): Promise<void> {
    ...
    if (state.document_identity_hash && state.document_identity_hash !== current) this.store.update({ document_identity_hash: current, active_command: null, command_status: "IDLE", active_view: "recognition", recognition_summary: "", paragraph_recognition_models: [], formatting_preview_models: [], preview_comment_status: "", formatting_progress: "已切换文档，等待操作", formatting_result: "", latest_error: "" });
  }
  ...
  private async preview(_result: CommandResult): Promise<string> { const identity = await this.documentIdentity(); const value = await this.composition().previewUseCase.execute(id("preview")); const grouped = new Map<number, FormattingCommandSet["commands"]>(); for (const command of value.commands.commands) { const list = grouped.get(command.target.source_paragraph_index) ?? []; list.push(command); grouped.set(command.target.source_paragraph_index, list); } const models = value.recognition.paragraphs.map((item) => ({ paragraph_index: item.source_paragraph_index, recognized_type: item.recognized_type, plan: formattingPlan(grouped.get(item.source_paragraph_index) ?? []), needs_review: item.needs_review })); this.store.update({ document_identity_hash: identity, formatting_preview_models: models, preview_comment_status: `已创建 ${value.summary.preview_comment_count ?? 0} 条临时批注`, active_view: "preview" }); return "预览排版完成"; }
...
async function runAutomaticHostAcceptance(application: ApplicationLike, router: HostCommandRouter, build: BuildInfo): Promise<void> {
  ...
  const steps: Array<[string, HostCommandName]> = [["taskpane_open", "open_taskpane"], ["taskpane_close", "close_taskpane"], ["taskpane_reopen", "open_taskpane"], ["host_recognize", "recognize_document"], ["host_preview", "preview_document"], ["one_click_format", "format_document"]];
  for (const [stage, command] of steps) { const result = await router.dispatch(command, "e2e", `e2e-${stage}-${Date.now().toString(36)}`, build.build_id); await reportHostAcceptance(stage, result.status === "PASS" ? "PASS" : "FAIL", result.error_code); if (result.status !== "PASS") return; }
  application.ActiveDocument?.Save?.(); await reportHostAcceptance("host_command_router", "PASS");
}
...
try { install(); } catch (error) { const application = hostWindow.Application; const build = hostWindow.DocxtoolBuildInfo; if (application && build) new HostResultStore(application.PluginStorage, build).update({ latest_error: stableError(error), active_view: "issues" }); }

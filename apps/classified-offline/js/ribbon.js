...
DocxtoolRibbonLog("INFO", "ribbon.script.loaded", "开发 Ribbon 脚本已执行", { application_available: Boolean(window.Application) });
...
function OnAddinLoad(ribbonUI) {
  ...
  try {
    ...
    if (typeof window.DocxtoolReportBootstrapStage === "function") void window.DocxtoolReportBootstrapStage("ribbon_onload", "PASS", "");
...
function DocxtoolFallbackShowTaskPane() {
  var pane = null; var key = "docxtool_classified_taskpane";
  DocxtoolRibbonLog("DEBUG", "taskpane.lookup.start", "开始查找已存在的任务窗格", {});
  try {
    var saved = window.Application.PluginStorage.getItem(key);
    if (saved) {
      try { pane = window.Application.GetTaskPane(Number(saved)); DocxtoolRibbonLog("DEBUG", "taskpane.lookup.success", "已完成任务窗格查找", { found: Boolean(pane) }); }
      catch (error) { DocxtoolRibbonLog("WARN", "taskpane.lookup.failed", "读取已保存任务窗格失败", {}, error); pane = null; }
    } else DocxtoolRibbonLog("DEBUG", "taskpane.lookup.success", "没有已保存的任务窗格", { found: false });
    if (!pane) {
      DocxtoolRibbonLog("INFO", "taskpane.create.start", "开始创建任务窗格", {});
      try {
        pane = window.Application.CreateTaskPane(GetUrlPath() + "/ui/taskpane-development.html", "Docxtool 涉密版");
        ...
        DocxtoolPersistHostError("DocxtoolFallbackShowTaskPane", "TASKPANE_CREATE_FAILED"); return false;
      }
    }
    try { pane.Visible = true; if (!pane.Visible) throw new Error("TASKPANE_VISIBLE_FALSE"); DocxtoolRibbonLog("INFO", "taskpane.show.success", "任务窗格已显示", {}); return true; }
    catch (error) { DocxtoolRibbonLog("ERROR", "taskpane.show.failed", "任务窗格显示失败", { stable_error_code: "TASKPANE_SHOW_FAILED" }, error); return false; }
  } catch (error) { DocxtoolRibbonLog("ERROR", "taskpane.lookup.failed", "任务窗格兜底流程失败", {}, error); return false; }
}

function OnAction(control) {
  var started = Date.now(); var controlId = control && control.Id ? String(control.Id) : ""; var command = DOCXTOOL_RIBBON_COMMANDS[controlId];
  var requestId = "ribbon-" + Date.now().toString(36) + "-" + Math.random().toString(16).slice(2, 10); var buildId = window.DocxtoolBuildInfo ? window.DocxtoolBuildInfo.build_id : "unknown";
  ...
  if (!command) { DocxtoolRibbonLog("WARN", "ribbon.action.unknown", "收到未知功能区按钮", { control_id: controlId, request_id: requestId }); DocxtoolPersistHostError("OnAction:" + controlId, "RIBBON_CALLBACK_NOT_FOUND"); DocxtoolFallbackShowTaskPane(); return true; }
  if (typeof window.DocxtoolHostDispatch !== "function") { DocxtoolRibbonLog("ERROR", "ribbon.dispatch.unavailable", "主宿主命令路由尚未就绪", { control_id: controlId, command_name: command, request_id: requestId, correlation_id: requestId, stable_error_code: "HOST_COMMAND_ROUTER_NOT_READY" }); DocxtoolPersistHostError("OnAction:" + controlId, "HOST_COMMAND_ROUTER_NOT_READY"); DocxtoolFallbackShowTaskPane(); return true; }
  ...
  try {
    Promise.resolve(window.DocxtoolHostDispatch(command, "ribbon", requestId, buildId)).then(function (result) {
      ...
      DocxtoolPersistHostError("OnAction:" + controlId, "HOST_COMMAND_FAILED"); DocxtoolFallbackShowTaskPane();
    });
  } catch (error) {

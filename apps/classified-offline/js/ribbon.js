...
function OnAction(control) {
  ...
  if (typeof window.DocxtoolHostEnqueue !== "function") { DocxtoolPersistHostError("OnAction:" + controlId, "HOST_COMMAND_QUEUE_NOT_READY"); return true; }
  try {
    var receipt = window.DocxtoolHostEnqueue(command, "ribbon");
    DocxtoolRibbonLog("INFO", "ribbon.command.enqueued", "命令已提交到本地队列", { control_id: controlId, command_name: command, command_id: receipt && receipt.command_id ? receipt.command_id : "", accepted: Boolean(receipt && receipt.accepted), reason: receipt && receipt.reason ? receipt.reason : "" });
  } catch (error) {

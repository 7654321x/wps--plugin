...
Codex 为构建、验证、调试或授权操作临时启动的后台进程、监听服务和临时授权会话，完成后必须主动关闭或撤销，并确认没有遗留进程继续占用资源。不得结束用户原本已经启动或正在使用的 WPS、Broker、资源服务、控制服务及其他程序；无法确认归属时只报告，不擅自停止。
...
本项目默认以当前明确功能真实跑通为第一目标。只处理当前需求、已复现故障和会直接阻断主流程的问题，不臆想尚未发生的风险，不为理论场景增加防御分支、兼容路径、重试、降级、抽象层或额外门禁。

- 出现错误时优先暴露并修复真实根因，不通过默认值、静默跳过、宽泛异常捕获或绕过逻辑让流程表面通过。
- 除发布固定门禁、当前正常主流程、已发现真实失败和防止本次问题复发的关键测试外，不追加全量审计、重复测试或假设性边界测试。
- 不进行与当前功能无关的安全加固、架构治理、目录调整、依赖升级、全文件格式化或清理；范围外问题只记录，不自行实施。
- 已有身份、合同、数据完整性和不可逆操作门禁按真实故障边界保留；不得以“更安全”为由继续叠加未经需求或故障证明的校验。
- 测试与明确需求或真实行为冲突时，先核实测试是否过时；不得仅为通过错误测试而破坏正确功能。

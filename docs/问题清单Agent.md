...
## P066 启动状态绕过统一日志并逐项刷屏

- 症状：启动结束后连续输出十余条 `print` 状态、单独的完成提示和日志路径；“项目版本”实际显示 Git 提交号，用户难以快速判断整体状态。
- 根因：`print_status()`、启动完成分支和日志监视入口分别直接写控制台，没有经过统一事件格式、分组和去重边界。
- 禁止：为每个状态项各写一条日志；在统一日志之外重复输出完成提示或日志路径；把提交摘要标成产品版本；让多行摘要绕过脱敏。
- 正确方案：状态收集完成后生成一个 `launcher.status.summary` 事件，首行保留时间、级别、组件和事件码，后续缩进行按版本与插件、资源与注册、本地识别、控制服务、旧链路、执行方式和下一步分组；产品版本与 Git 提交分别显示。多行内容逐行经过统一脱敏器。
- 自动验证门槛：格式器测试确认一个事件可输出多行、首行仍可解析、路径和授权值被脱敏；`print_status()` 只调用一次 `log_event` 且无直接控制台输出；真实 `main.py status` 只显示一个状态汇总事件，内容与当前服务一致。

## P067 WPS 完整加载后缺少单一完成事件

- 症状：日志分别出现首页、Bootstrap、Ribbon、Host 和 Worker 成功事件，但没有明确的一条“WPS 加载完成”结论。
- 根因：各宿主上下文独立发送阶段事件，启动器只判断页面是否加载，没有在当前构建的必需阶段集合满足后汇总状态。
- 禁止：只凭首页 200 或单个脚本事件写加载成功；缺少阶段时输出假成功；重复写入 `wps.load.completed`。
...
## P068 WPS Range 数值坐标与字符边界不能混用

- 症状：Broker 已完成识别且段落全文、Host UTF-16 子片段哈希均正确，但 Worker 写预览批注时返回 `HOST_RANGE_TEXT_MISMATCH`，WPS 中没有批注。
- 根因：识别锚点是 Host 文本中的 UTF-16 边界，但 WPS 写入边界曾用 `Paragraph.Range.Start + 偏移` 计算数值坐标。v1.5.10 实机日志证明 WPS 接受了请求坐标且返回相同长度，读回哈希仍不同，因此 `Paragraph.Range.Start/End` 数值不能直接充当该文本的字符边界。
...
## P071 Worker 预览清理用户批注完整性失败缺少阶段证据

- 症状：一键排版返回 `PREVIEW_USER_COMMENT_CHANGED`，顶层只有一条失败日志，无法判断失败发生在批次开始、删除批注、剩余数量还是完整性比较。
- 根因：Worker 预览清理是 Host Runtime 直接调用，未经过 Host RPC；原有清理函数没有把当前文档 token、批次数量和用户批注完整性结果写入统一日志。
- 禁止：删除用户批注保护、忽略 `PREVIEW_USER_COMMENT_CHANGED`、记录用户批注正文/完整指纹/完整 token，或只把错误包装成“排版失败”。
- 正确方案：清理开始、每个批次开始/完成、删除异常、指纹不匹配、HostBridge 汇总失败和最终失败均记录阶段、批次、数量、token/会话后缀和稳定错误码；正文、完整指纹和完整令牌继续脱敏。
- 自动验证门槛：Node 回归确认清理成功有批次 start/completed 日志；跨文档、删除异常和用户完整性失败均有明确错误阶段；真实 WPS 复现时日志可确定实际失败位置后再修复根因。

## P072 一键排版不应强制删除预览批注

- 症状：一键排版为了删除临时预览批注执行额外的 WPS 批注扫描和删除，遇到 `PREVIEW_USER_COMMENT_CHANGED` 阻断排版。
- 根因：预览批注本身不影响正文识别和格式命令，排版流程却把“删除预览”绑定成前置条件，增加了性能和 WPS 批注 API 失败点。
- 禁止：一键排版前自动删除预览批注、因批注删除失败而阻断正文排版，或把保留批注报告成“无预览批注”。
- 正确方案：一键排版直接保存当前文档、执行正式排版并再次保存；预览批注保留在文档中，用户通过“清除预览”主动删除。重复预览仍由预览流程负责清理旧预览，避免重复叠加。
- 自动验证门槛：格式流程不调用 Worker 预览清理；保存、识别和排版继续执行；完成状态明确显示“预览批注已保留”；清除预览按钮仍能单独删除并保护用户批注。

## P073 一键排版队列生产者与 Broker 路径合同分叉

- 症状：任务写入文件队列后，Broker 领取阶段返回 `LOCAL_JOB_REQUEST_PATH_INVALID`；预览链路可用而一键排版链路失败。
- 根因：旧的 `LocalProcessRecognitionTransport` 自行拼接 WPS AppData 队列路径，正式 Worker 使用另一套配置路径；Broker 对结果和错误路径进行原始字符串比较，路径物理位置相同但文本表示不同也会被拒绝。
- 禁止：删除 Broker 的输出路径边界、接受任务目录外路径、让一键排版重新回到独立 ShellExecute 或旧端口服务。
- 正确方案：正式排版统一使用 Worker 与已配置的 Broker 队列；Broker 对 Windows 路径进行规范化后比较，并保留任务目录边界；错误日志只记录字段匹配摘要，不记录完整本机路径。
- 自动验证门槛：TypeScript 任务生产与 Python Broker 跨语言领取回归通过；等价分隔符/规范化路径可领取；任务目录外路径继续返回 `LOCAL_JOB_REQUEST_PATH_INVALID`。

## P074 正式排版不能在主线程走同步全文扫描

- 症状：点击“一键排版”后 WPS 长时间无可见进度；首次快照已耗时约 19.7 秒，失败前还未进入格式写入。
- 根因：一键排版仍调用同步 `FormatDocumentUseCase`，Worker 虽声明 `format` 合同却拒绝该命令；WPS 主线程重复读取段落、格式修订和全文 activeRevision。
